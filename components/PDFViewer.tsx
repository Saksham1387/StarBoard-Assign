...
// export default function EnhancedPDFCSVViewer({fileUrl, pageNumber, highlightText}: ViewerProps) {
...
//   // Parse column and value from CSV highlight text
//   const parseCSVHighlightData = useCallback(() => {
//     const textToHighlight = getHighlightText();
//     if (!textToHighlight) return { columnName: '', valueToHighlight: '' };

//     // Parse format: "Lease Start: 01/05/2022" -> column: "Lease Start", value: "01/05/2022"
//     const match = textToHighlight.match(/^([^:]+):\s*(.+)$/);
//     if (match) {
//       return {
//         columnName: match[1].trim(),
//         valueToHighlight: match[2].trim()
//       };
//     }

//     // Fallback to original logic if format doesn't match
//     return { columnName: '', valueToHighlight: textToHighlight };
//   }, [getHighlightText]);

//   // Enhanced CSV highlighting function
//   const highlightCSVText = useCallback(() => {
//     if (!csvRef.current || !csvData) return;

//     const { columnName, valueToHighlight } = parseCSVHighlightData();
//     if (!valueToHighlight) return;

//     const csvContainer = csvRef.current;

//     // If we have a column name, find the specific column and highlight only values in that column
//     if (columnName) {
...
//         if (headerCell && headerCell.textContent?.toLowerCase().trim() === columnName.toLowerCase().trim()) {
//           const headerText = headerCell.textContent;
//           const regex = new RegExp(`(${columnName.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')})`, 'gi');
//           headerCell.innerHTML = headerText.replace(regex,
...
//             if (cellText.toLowerCase().includes(valueToHighlight.toLowerCase())) {
//               const regex = new RegExp(`(${valueToHighlight.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')})`, 'gi');
//               const highlightedHTML = cellText.replace(regex,
//                 `<span class="csv-highlight" style="background-color: ${highlightColor}; padding: 1px 2px; border-radius: 2px; color: black; font-weight: 500;">$1</span>`
//               );
...
//   // Render CSV view (keeping original logic)
//   if (isCSVMode) {
//     return (
...
//                 return columnName ? (
//                   <>
//                     Highlighting column: <span className="font-mono bg-blue-100 px-2 py-1 rounded">{columnName}</span>
//                     {" "}with value: <span className="font-mono bg-yellow-100 px-2 py-1 rounded">{valueToHighlight}</span>
//                   </>
//                 ) : (
//                   <>
//                     Highlighting: <span className="font-mono bg-gray-100 px-2 py-1 rounded">{getHighlightText()}</span>
//                   </>
//                 );
//               })()}
//             </p>
...
//       <Document
//         file={fileUrl || "https://assignment-starbaord.s3.ap-south-1.amazonaws.com/uploads/04d34a7e-b431-4191-ab42-4afaec0e6f0b-280+Richards+-+OM.pdf"}
  ...
  return (
    <div className="flex items-center justify-center flex-row pdf-viewer-container">
      <Document
        file={
          fileUrl ||
          "https://assignment-starbaord.s3.ap-south-1.amazonaws.com/uploads/04d34a7e-b431-4191-ab42-4afaec0e6f0b-280+Richards+-+OM.pdf"
        }
        onLoadSuccess={onDocumentLoadSuccess}
      >
        <div ref={pageRef}>
